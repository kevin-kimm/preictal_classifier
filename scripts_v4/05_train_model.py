"""
=============================================================
  Siena Scalp EEG — Model Training v4
  05_train_model.py

  Changes vs v3:
    + 10-minute preictal window (was 5 min)
    + 60-second EEG windows (was 30 sec)
    + Metal GPU acceleration for Apple Silicon
    + Larger batch size (256) optimized for M4

  Loads:   data/features/features_v4.npz
  Method:  Leave One Patient Out (LOPO) cross-validation
  Models:  1) GradientBoosting
           2) Dense neural network with focal loss

  Output:  models_v4/lopo_results.json
           models_v4/gb_<patient>.pkl
           models_v4/nn_<patient>.keras
=============================================================
"""
...
# Enable Metal GPU acceleration on M4 chip
...
FEATURES_PATH = Path("data/features/features_v4.npz")
...
def build_gb() -> GradientBoostingClassifier:
    ...
    Note: CPU only — does not use Metal GPU.
    """
...
def build_nn(n_features: int) -> keras.Model:
    """
    Larger network than v2 — appropriate for 287 features.
...
# LOPO training loop
print("\n" + "=" * 62)
print("  LEAVE ONE PATIENT OUT — v4")
print("=" * 62)
