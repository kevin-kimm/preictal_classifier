...
# CONFIG
PATIENT      = "PN07"   # PN05 PN07 PN10 PN12 PN13 PN14 PN16 PN17
PATIENT_AUCS = {
...
THRESHOLD       = 0.65
DISPLAY_CHANNEL = "T3"
DISPLAY_SEC     = 120      # 2 min rolling EEG window
SPEED           = 15.0

# Demo starts 1 min before alert fires, ends 3 min after seizure
DEMO_PRE_ALERT  = 60       # seconds before alert to start demo
DEMO_POST_SEC   = 3 * 60   # seconds after seizure end
...
# Colors
C_BG="white"; C_EEG="#2166ac"; C_NORMAL="#1a9641"
C_CAUTION="#d97f00"; C_ALERT="#d7191c"; C_SEIZURE="#7b2d8b"
C_END="#228B22"; C_TEXT="#1a1a1a"; C_GRID="#e8e8e8"
C_THRESH="#d97f00"
...
def load_and_preprocess(edf_path, cm):
    """Load one EDF, apply full preprocessing, return raw+norm signals."""
...
def compute_baseline(norm_sig, nch, minutes=20):
    """Compute baseline mean/std from first N minutes."""
    bf=[]; s=0
...
# LOAD MODEL
print(f"\n{'='*62}\n  SEIZURE PREDICTION DEMO — {PATIENT}\n{'='*62}")
...
for edf in sorted(pdir.glob("*.edf")):
    ...
    nch_=nd.shape[0]
    dci_=next((i for i,c in enumerate(cnames)
...
if best["sz_onset"] is None:
    ...
    for edf in sorted(pdir.glob("*.edf")):
        ...
        best.update({
            ...
            "raw_sig":rd,"norm_sig":nd,"nch":nch_,
            "dci":dci_,"bmean":bm_,"bstd":bs_,
        })
        break
...
alert_t  = best["alert_t"]  # when alert SHOULD fire (from scan)
horizon  = best["horizon"]
pre_start= sz_onset - PREICTAL_SEC
total    = norm_sig.shape[1]

# Demo starts 1 min before alert fires
# This guarantees the audience sees the alert fire during the demo
...
print(f"  Demo starts at : t={demo_start:.0f}s  "
      f"(1 min before alert)")
print(f"  Demo ends at   : t={demo_end/TARGET_SFREQ:.0f}s")
...
# EEG
ax_eeg.set_title(f"EEG — Channel {DISPLAY_CHANNEL}",
                  fontsize=12,fontweight="bold")
ax_eeg.set_xlabel("Time (seconds)",fontsize=9)
ax_eeg.set_ylabel("Amplitude (uV)",fontsize=9)
ax_eeg.set_ylim(-150,150)
ax_eeg.grid(True,alpha=0.4)
eeg_line,=ax_eeg.plot([],[],color=C_EEG,lw=0.7,zorder=3)
...
lbl_alert =ax_eeg.text(0,128,"ALERT",  color=C_ALERT,  fontsize=8,
                        fontweight="bold",visible=False,zorder=6)
lbl_sz_on =ax_eeg.text(0,128,"SEIZURE",color=C_SEIZURE,fontsize=8,
                        fontweight="bold",visible=False,zorder=6)
lbl_sz_end=ax_eeg.text(0,128,"END",    color=C_END,    fontsize=8,
                        fontweight="bold",visible=False,zorder=6)
...
fig.suptitle(
    f"Seizure Prediction  |  Patient {PATIENT}  |  "
    f"v6 Neural Network  |  Threshold {THRESHOLD}",
...
raw_init=raw_sig[dci,i0:ds].copy()
if np.max(np.abs(raw_init))<0.01: raw_init*=1e6
state["eeg_buf"].extend(raw_init.tolist())
...
def update(frame):
    ...
    # Feed EEG
    e0=min(ns+adv,total)
    raw_new=raw_sig[dci,ns:e0].copy()
    if np.max(np.abs(raw_new))<0.01: raw_new*=1e6
    state["eeg_buf"].extend(raw_new.tolist())
    ...
    while (state["cur"]-state["last_win"])>=STEP_SAMPLES:
        ...
        # Alert fires when model crosses threshold
        if prob>=THRESHOLD and not state["alerted"]:
            state["alerted"]=True
            state["alert_fired_t"]=t   # display time when alert fired
            h=sz_onset-wt
    ...
    if len(ta)>1:
        ...
        if state["alerted"] and not state["alert_drawn"]:
            ...
            lbl_alert.set_position((state["alert_fired_t"]+2,128))
        ...
        if t>=sz_onset and not state["sz_drawn"]:
            ...
            lbl_sz_on.set_position((sz_onset+2,128))
        ...
        if t>=sz_end and not state["sz_end_drawn"]:
            ...
            lbl_sz_end.set_position((sz_end+2,128))
    ...
    if t>=sz_end:
        ...
        detail_txt.set_text(
            f"Predicted {horizon/60:.1f} min early"
            if horizon>0 else "")
