"""
=============================================================
  Siena Scalp EEG — Preprocessing Pipeline v6 Headband
  03_preprocess.py

  Hybrid montage — headband channels + F7, F8:
    Channels  : F7, T3, T5, O1, Pz, O2, T6, T4, F8  (9ch)
    Window    : 30 seconds
  ...
  Output: data/processed_v6_headband/
=============================================================
"""
...
# Suppress MNE's verbose output — it prints a lot by default
mne.set_log_level("WARNING")


# ── File paths ────────────────────────────────────────────
# Where the raw Siena dataset lives
DATA_ROOT  = Path("data/siena-scalp-eeg-database-1.0.0")

# Where to save processed windows per patient
OUTPUT_DIR = Path("data/processed_v6_headband")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


# Configuration constants 
# Hybrid montage: headband-friendly channels PLUS F7 and F8
...
# O1, O2 : occipital — standard headband channels
# Pz     : parietal midline — standard headband channel
TARGET_CHANNELS = ["F7", "T3", "T5", "O1", "Pz",
                   "O2", "T6", "T4", "F8"]
...
# We need to convert these to seconds from recording start
...
def process_edf(edf_path, seizures_in_file, channel_map):
    ...
    # Keep only our 9 target channels
    ...
    # X shape: (n_windows, 9_channels, 7500_samples)
...
print("\n" + "=" * 62)
print("  PREPROCESSING v6 HEADBAND")
print("=" * 62)
...
for pdir in patient_dirs:
    ...
    print(f"  Channels : {len(channel_map)}/9 -> "
